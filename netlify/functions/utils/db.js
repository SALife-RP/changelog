    ...
    try {
        ...
        const characterData = await Promise.all(
            characters.map(async(charId) => {
                ...
                const [identityResults, moneyResults, vehicleResults] =
                await Promise.all([
                    pool.execute(
                        "SELECT * FROM salrp_user_identities WHERE user_id = ?", [userId]
                    ),
                    pool.execute(
                        "SELECT cash, debt FROM banking2_accounts WHERE user_id = ?", [userId]
                    ),
                    pool.execute(
